import { userManagementService, User, UserRole } from './userManagementService';
...
export const authService = {
   ...
   * Log login attempt
   */
  logLoginAttempt(user_id: string, email: string, action: string): void {
    try {
      const logs: LoginLog[] = JSON.parse(
        localStorage.getItem(LOGIN_LOG_KEY) || '[]'
      );
