...
export interface UserWithKaryawan extends User {
  karyawan?: {
    id: string;
    nama: string;
    jabatan: string;
...
export const userManagementService = {
   ...
   * Create new user
   */
  async createUser(input: CreateUserInput): Promise<{ user_id: string; success: boolean }> {
    try {
      // Validate input
   ...
   */
  async changePassword(user_id: string, oldPassword: string, newPassword: string): Promise<boolean> {
    try {
      // Get user
   ...
   */
  async getUserById(user_id: string): Promise<UserWithKaryawan | null> {
    try {
      const { data, error } = await supabase
        .from('users')
        .select('*')
      ...
      let userWithDetails: UserWithKaryawan = data;
   ...
   */
  async getManagers(): Promise<Array<{ id: string; nama_lengkap: string }>> {
    try {
      const { data, error } = await supabase
        .from('users')
        .select('id, nama_lengkap')
   ...
   * Update user
   */
  async updateUser(user_id: string, updates: Partial<User>): Promise<boolean> {
    try {
      const { error } = await supabase
        .from('users')
        .update({
   ...
   * Update user role
   */
  async updateUserRole(user_id: string, newRole: UserRole): Promise<boolean> {
    try {
      const { error } = await supabase
        .from('users')
        .update({
   ...
   * Reactivate user
   */
  async reactivateUser(user_id: string): Promise<boolean> {
    try {
      const { error } = await supabase
        .from('users')
        .update({
   ...
   */
  canUserAccess(userRole: UserRole, requiredRole: UserRole[]): boolean {
   ...
   */
  canApprovePayroll(userRole: UserRole): boolean {
   ...
   */
  canCreatePayroll(userRole: UserRole): boolean {
   ...
   */
  canViewPayroll(userRole: UserRole, isOwnPayroll: boolean): boolean {
