...
export interface PayrollRecord {
  ...
  periode_end: string;
  frequency_type: 'weekly' | 'bi_weekly' | 'monthly';
  gaji_pokok: number;
  bonus_on_time: number;
...
export interface PayrollDeduction {
  id?: string;
  payroll_id?: string;
  type: 'late' | 'absent' | 'custom' | 'pph21' | 'insurance' | 'pension' | 'loan';
  description: string;
  amount: number;
  notes?: string;
  created_at?: string;
...
export interface PayrollCalculation {
  ...
  deduction_custom: number;
  tax_pph21: number;
  total_deductions: number;

  net_salary: number;

  details: PayrollDeduction[];
}
...
export const payrollService = {
   ...
   */
  async getTaxSettings(): Promise<{ nontaxable: number; brackets: TaxBracket[] }> {
    try {
   ...
   * Calculate PPh21 tax
   */
  calculatePPh21(grossSalary: number, taxSettings: { nontaxable: number; brackets: TaxBracket[] }): number {
   ...
   */
  async getBonusDeductionRules(): Promise<{
    bonusOnTime: number;
    bonusEarlyCheckout: number;
    ...
    try {
      ...
      const bonusOnTime =
        bonusRules.data?.find((r) => r.tipe === 'on_time')?.jumlah || 50000;
      const bonusEarlyCheckout =
        bonusRules.data?.find((r) => r.tipe === 'early_checkout')?.jumlah || 25000;

      const deductionLate =
        deductionRules.data?.find((r) => r.tipe === 'late')?.jumlah || 10000;
      const deductionAbsent =
        deductionRules.data?.find((r) => r.tipe === 'absent')?.jumlah || 100000;
  ...
  async calculatePayroll(
    ...
    periode_start: string,
    periode_end: string,
    customDeductions: Array<{ description: string; amount: number }> = []
    ...
    try {
      ...
      const details: PayrollDeduction[] = [];
  ...
  ): Promise<{ payroll_id: string; success: boolean }> {
    try {
   ...
   * Get payroll by ID
   */
  async getPayrollById(payroll_id: string): Promise<PayrollRecord | null> {
    try {
      const { data, error } = await supabase
        .from('payroll_records')
        .select('*')
   ...
   */
  async deletePayroll(payroll_id: string): Promise<boolean> {
    try {
      const { error } = await supabase
        .from('payroll_records')
        .delete()
